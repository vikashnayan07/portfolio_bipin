...
/* ═══════════════════════════════════════════════════════════════════════════
   CONVERSATION PANEL — Standalone component (NOT inside MessagesManager)
   This prevents React from unmounting/remounting on every keystroke,
   which was causing the textarea to lose focus after typing one character.
   ═══════════════════════════════════════════════════════════════════════════ */
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-xs">{selectedMsg.email}</p>
            </div>
          </div>
...
const MessagesManager = () => {
  ...
  // Props shared by both mobile & desktop ConversationPanel instances
    ...
    <div className="max-w-7xl mx-auto">
      ...
      {mobileView === "chat" && selectedMsg && (
        <div className="lg:hidden">
          <ConversationPanel {...conversationProps} isFullScreen={true} />
        </div>
      )}

      {/* Main layout */}
      <div
        className={`flex gap-6 ${mobileView === "chat" ? "hidden lg:flex" : ""}`}
      >
        {/* Messages List */}
